...
import random
class dataSet:
	def __init__(self,text_path,train_graph_path,val_graph_path,train_y_path,val_y_path,val_q_path,demo_graph_path,demo_y_path):

		text_file,train_graph_file,val_graph_file,train_y_flie,val_y_flie,val_q_file,demo_graph_file,demo_y_file=self.load(text_path,train_graph_path,val_graph_path,train_y_path,val_y_path,val_q_path,demo_graph_path,demo_y_path)

		self.train_edges=self.load_edges(train_graph_file)
		self.val_edges = self.load_edges(val_graph_file)
		self.demo_edges = self.load_edges(demo_graph_file)
		self.demo_y=self.load_y(demo_y_file)
	...
	def load(self,text_path,train_graph_path,val_graph_path,train_y_path,val_y_path,val_q_path,demo_graph_path,demo_y_path):
		...
		demo_graph_file = open(demo_graph_path, 'rb').readlines()
		demo_y_file = open(demo_y_path, 'rb').readlines()
	
		return text_file,train_graph_file,val_graph_file,train_y_flie,val_y_flie,val_q_flie,demo_graph_file,demo_y_file
	...
	def generate_batches(self,mode=None):
		if mode =="demo":
			edges = self.demo_edges
			ans=[]
			for ed in edges:
				ans.append(ed[2])
			ys=self.demo_y
			ey = list(zip(edges, ys,ans))
			i=0
			num=len(ey)
			while len(ey)+num<config.batch_size:
				ey.extend(ey)
			ey.extend(ey[:(config.batch_size-len(ey) % config.batch_size)])
			num_batch=len(ey) // config.batch_size

		if mode != 'validation' and  mode !="demo":
		...
		if mode=='demo':
			anns=[]
			sample_ans=[]
			sample_edges[:], sample_ys[:] ,sample_ans[:]= zip(*sample_ey)
			for i in range(num_batch):
				batches.append(sample_edges[i*config.batch_size:(i+1)*config.batch_size])
				y.append(sample_ys[i*config.batch_size:(i+1)*config.batch_size])
				anns.append(sample_ans[i*config.batch_size:(i+1)*config.batch_size])
			return batches, y,num,anns

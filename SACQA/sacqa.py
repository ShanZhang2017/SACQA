...
class Model:
    def __init__(self,vocab_size):
        ...
        self.loss,self.prediction =self.compute_loss_acc()
    ...
    def conv(self):
        ...
        rand_matrix1=tf.Variable(tf.truncated_normal([config.embed_size,config.embed_size],stddev=0.3))
        rand_matrix2 = tf.Variable(tf.truncated_normal([config.embed_size, config.embed_size], stddev=0.3))
...
# Q和正确答案
    ...
    def multilayer_perception(self,inputs_1,in_size, out_size, activation_function=None):
    ...
    def compute_loss_acc(self):
        ...
        l_r1 = self.multilayer_perception(rs,800, 256, activation_function=tf.nn.sigmoid)  # 隐藏层
        # l_r2  = self.multilayer_perception(l_r1, 256, 256, activation_function=tf.nn.sigmoid)
        prediction_r = self.multilayer_perception(l_r1, 256, 1, activation_function=None)
